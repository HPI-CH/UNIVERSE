...
def _ppg_findpeaks_elgendi(
 ...
 #   print("**** threshold**** ", thr1)
   ...
   # print(ma_peak)
    true_count = np.sum(waves)
    print("Number of times 'waves' is True:", true_count)
    beg_waves = np.where(np.logical_and(np.logical_not(waves[0:-1]), waves[1:]))[0]
    #print("beg_waves", beg_waves)
    end_waves = np.where(np.logical_and(waves[0:-1], np.logical_not(waves[1:])))[0]
   # print("end_waves", end_waves)
    if len(end_waves) > 0 and len(beg_waves) > 0: ## I have changed this to avoid no peaks detection error
        ...
        peaks = np.array([0])
        print("changed by Sidratul sothat whenevers there are not enough data points above the threshold then it can provide with 0 peaks")
    return peaks
